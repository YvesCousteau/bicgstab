...
def petsc(mat_A,mat_b,mat_x,n,tol,it_max,pc,cond,yaml_data):
    ...
    ksp.setTolerances(rtol=tol,max_it=it_max)
    ...
    print((t2-t1)*1000.0)
    # ksp.view()
    residual = str((norm(mat_b - mat_A*x_petsc)/norm(mat_b)))
    forward_error = str((norm(mat_x - x_petsc)))
    cond_backward_error = str((cond*((norm(mat_b - mat_A*x_petsc)/norm(mat_b)))))
    yaml_data["petsc"] = {
